...
ViewEdit::ViewEdit(wxFrame* parent) :wxScrolledCanvas(parent, wxID_ANY)
{
    ...
    parent->Bind(wxEVT_COMMAND_MENU_SELECTED, &ViewEdit::OnEditMachine1Number, this, XRCID("SelectMachine1"));
    parent->Bind(wxEVT_COMMAND_MENU_SELECTED, &ViewEdit::OnEditMachine2Number, this, XRCID("SelectMachine2"));
...
/**
 * Handle an Edit>Machine 1 menu option
 * @param event The menu event
 */
void ViewEdit::OnEditMachine1Number(wxCommandEvent& event)
{
    GetPicture()->GetMachine1()->ShowMachineDlg(this->GetParent());
}

/**
 * Handle an Edit>Machine 2 menu option
 * @param event The menu event
 */
void ViewEdit::OnEditMachine2Number(wxCommandEvent& event)
{
    GetPicture()->GetMachine2()->ShowMachineDlg(this->GetParent());
}
